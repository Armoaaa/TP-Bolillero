...
public class Simulacion
{
    ...
    public async Task<long> SimularConHilosAsync(Bolillero bolillero, IReadOnlyList<int> jugada, int cantidadSimulaciones, int cantidadHilos)
    {
        if (bolillero is null)
        {
            throw new ArgumentNullException(nameof(bolillero));
        ...
        for (var i = 0; i < cantidadHilos; i++)
        {
            ...
            {
                var copiaBolillero = bolillero.Clonar();
                var ganadasParciales = await copiaBolillero.JugarNVecesAsync(jugada, simulacionesEnHilo);
                Interlocked.Add(ref ganadas, ganadasParciales);
            }));
        }
